 ...
 * protocol switch that should clear it get rejected first.
 *
...
const Config = z.object({
	 ...
	 */
	defaultEffort: z.union([z.const(false), ...THINKING_LEVELS]).default("medium"),
...
/**
 * Compute a route-level `compat` op for protocol switches. On
 * `openai-completions` a DeepSeek-style route gets `thinkingFormat` backfilled
 * (so the adapter sends the DeepSeek `thinking` wire spelling instead of the
 * OpenAI `reasoning_effort` default); on any other protocol the
 * completions-only switches are stripped so they cannot fail resolution.
 * `supportsDeveloperRole` and every other compat key are left untouched.
 ...
 */
function planCompatOp(route, compat, api, isDeepSeek, thinkingFormat) {
	if (api === "openai-completions") {
		if (thinkingFormat !== false && thinkingFormat !== void 0 && isDeepSeek) {
			const current = typeof compat === "object" && compat !== null && !Array.isArray(compat) ? compat : {};
			if (current.thinkingFormat === void 0) {
				return { op: "set", path: ["providers", route, "compat"], value: { ...current, thinkingFormat } };
			}
		}
		return null;
	}
	if (typeof compat === "object" && compat !== null && !Array.isArray(compat)) {
		if (compat.thinkingFormat !== void 0 || compat.supportsReasoningEffort !== void 0) {
			const next = { ...compat };
 ...
 */
function planOps(user, declaredRoutes, catalogModelIds, levels, maxTokens, contextWindow, thinkingFormat, defaultEffort) {
	const ops = [];
	const injected = [];
	...
	for (const [route, profile] of Object.entries(providers)) {
		...
		const compatOp = planCompatOp(route, profile.compat, api, isDeepSeek, thinkingFormat);
...
function apply(ctx, config = {}) {
	const { enabled = true, scope = "declared", levels = DEFAULT_LEVELS, verify = false, maxTokens = 384000, contextWindow = 1000000, thinkingFormat = false, defaultEffort = "medium" } = config;
	if (!enabled) return;
	...
	const refresh = () => queue(async () => {
		...
		const { ops, injected } = planOps(user, declaredRoutes, catalogModelIds, levels, maxTokens, contextWindow, thinkingFormat, defaultEffort);
		if (ops.length > 0) {
			try {
