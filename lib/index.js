 ...
 */
function planOps(user, declaredRoutes, catalogModelIds, levels, maxTokens, contextWindow, thinkingFormat) {
	const ops = [];
	const injected = [];
...
function apply(ctx, config = {}) {
	const { enabled = true, scope = "declared", levels = DEFAULT_LEVELS, verify = false, maxTokens = 384000, contextWindow = 1000000, thinkingFormat = false } = config;
	if (!enabled) return;
	...
	const refresh = () => queue(async () => {
		...
		const { ops, injected } = planOps(user, declaredRoutes, catalogModelIds, levels, maxTokens, contextWindow, thinkingFormat);
		if (ops.length > 0) {
			try {
